...
static void	find_pare2(t_list *tmp, char *pare)
{
	t_list	*tmp2;
	t_list	*tmp3;

	tmp2 = tmp;
	while (tmp2->next && (ft_strncmp((char *)tmp2->content, "sr", 3) == 0 \
	 || ft_strncmp((char *)tmp2->content, "sa", 3) == 0 \
	 || ft_strncmp((char *)tmp2->content, "sb", 3) == 0))
	{
		if (ft_strncmp((char *)tmp2->next->content, pare, 3) == 0)
		{
			((char *)tmp->content)[1] = 'r';
...
void	compression_inst(t_list **inst)
{
	...
	while (tmp->next)
	{
		if (ft_strncmp((char *)tmp->content, "ra", 3) == 0)
			...
			find_pare(tmp, "ra");
		else if (ft_strncmp((char *)tmp->content, "sa", 3) == 0)
			find_pare2(tmp, "sb");
		else if (ft_strncmp((char *)tmp->content, "sb", 3) == 0)
			find_pare2(tmp, "sa");
